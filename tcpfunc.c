...
/* 关闭指定客户端连接，关闭之后自动触发clientHandler中的删除节点操作,
 * 并关闭该客户端连接的线程
 * 输入参数：client_list *head,客户端列表头结点
 *           int sockfd,待删除的连接
 * 返回:int，关闭成功返回0，否则返回-1*/
int closeClient(client_list *head, int sockfd)
{
    int ret = 0;
    if ((NULL == head) || (sockfd < 1))
        return --ret;

    /* 如果成功关闭sockfd */
    if (!(close(sockfd)) && !(delClient(head, sockfd)))
    {
        printf("关闭客户端 %d 成功\n", sockfd);
        return ret;
    }
...
int closeClientFunc(client_list *head)
{
    ...
    int sockfd = 0;
    char order[orderlen];
    char exit_order[orderlen];

    setbuf(stdin, NULL);
    while (1)
    {
        setbuf(stdin, NULL);
        if (fgets(order, orderlen, stdin))
        {
            /* 输入--exit，退出当前功能，返回主页 */
            sscanf(order, "%s", exit_order);
            if (!(strcmp(exit_order, EXIT)))
            {
                memset(exit_order, 0, 10);
            ...
            /* 获取输入的sockfd */
            sscanf(order, "%d", &sockfd);
            /* sockfd有效性判断 */
            if (!(validSockfd(head, sockfd)))
                continue;
            ...
            printf("输入下列在线客户端的对应sockfd，关闭和它的连接\n");
...
/* 获取sockfd的有效性判断
 * 输入参数：client_list *head，客户端列表头结点
 *           int sockfd，待测试的sockfd
 * 返回值：int，如果是有效的，返回1，否则返回0*/
int validSockfd(client_list *head, int sockfd)
{
 ...
 /* 封装和发送在线客户端列表,
  * 输入参数：client_list *head,客户端列表头节点
  *           int sockfd，所在线程对应的客户端连接sockfd
  * 返回值：int，发送成功返回0，否则返回-1 */
int sendClist(client_list *head, int sockfd)
{
    ...
    while (cpt)
    {
        clist[len++] = cpt->sock_fd;
        cpt = (client_list *)(((node *)cpt)->next);
    }

    char str_send[MAX_BUF_SIZE];
    /* 加载头信息 */
    memcpy(str_send, CLIST_REQUEST_SEND, HEAD_BUF_SIZE - 1);
